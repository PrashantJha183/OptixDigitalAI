...
import Optix from "../../../assets/Optix Low Opacity Logo.svg"; // ensure file name has no spaces
...
const ImpactCard = memo(({ icon, title, stage, features }) => {
  ...
  return (
    <motion.article
      ...
      className="relative flex flex-col items-center text-center p-8 bg-white shadow-md rounded-xl transition-all duration-300 border-2 border-[#5d00c3] w-full max-w-xs md:max-w-sm"
    >
      ...
      {/* Responsive icon size — bigger on mobile */}
      <div className="text-[#5d00c3] text-5xl sm:text-5xl md:text-5xl lg:text-6xl mb-4">
        {icon}
      </div>

      <h3 className="text-xl md:text-2xl font-bold mb-4 text-[#5d00c3]">
        {title}
      </h3>
      ...
      <ul className="text-gray-700 text-base md:text-lg text-left space-y-2 mt-2">
        {features.map((feature, i) => (
          <li key={i} className="flex items-start space-x-2">
            <CheckCircle className="text-[#5d00c3] w-5 h-5 mt-1 flex-shrink-0" />
            <span>{feature}</span>
          </li>
        ))}
      </ul>
...
const MarketingImpact = () => {
  ...
  return (
    ...
    >
      {/* Background logo (visible on laptop+) */}
      <img
        src={Optix}
      ...
      <div className="max-w-7xl mx-auto text-center relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-10 md:gap-14 justify-items-center">
